...
def main():
	...
	if model_name.find('/') > -1:
		tblname = f'{model_name[model_name.find("/")+1:].replace("-", "_").replace(".", "")}_us_wce'
	else:
		tblname = f'{model_name.replace("-", "_").replace(".", "")}_us_wce'
...
class BalancedTrainer(Trainer):
	def __init__(self, **kwargs):
            ...
            self.class_count = np.array([freq[0][1], freq[1][1], 1, 1])
