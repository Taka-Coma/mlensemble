...
def main():
	...
	if model_name.find('/') > -1:
		tblname = f'{model_name[model_name.find("/")+1:].replace("-", "_").replace(".", "")}_us'
	else:
		tblname = f'{model_name.replace("-", "_").replace(".", "")}_us'
...
class BalancedTrainer(Trainer):
	def __init__(self, **kwargs):
	    ...
	    #self.class_count = np.array([freq[0][1], freq[1][1]])
            class_count = np.array([freq[0][1], freq[1][1], 0, 0])
            self.weight = 1./class_count

	def _get_train_sampler(self):
		sample_weight = torch.from_numpy(np.array([self.weight[t] for t in self.data_dict['labels']]))
