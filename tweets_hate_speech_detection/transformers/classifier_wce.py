...
def main():
	...
	if model_name.find('/') > -1:
		dbname = f'{model_name[model_name.find("/")+1:].replace("-", "_").replace(".", "")}_weighCE'
	else:
		dbname = f'{model_name.replace("-", "_").replace(".", "")}_weightCE'
...
class BalancedTrainer(Trainer):
	def __init__(self, **kargs):
		...
		self.weights = torch.from_numpy(weights).float().to('cuda')
	...
	def compute_loss(self, model, inputs, return_outputs=False):
		...
		loss_fn = torch.nn.CrossEntropyLoss(weight=self.weights)
